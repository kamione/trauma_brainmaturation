...
corr_res_p_label <- if_else(corr_res$p.adj < 0.001, "~italic(p)<0.001", 
                            as.character(glue("~italic(p)=={round(corr_res$p.adj, 3)}")))
corr_res_label <- paste(corr_res_r_label, corr_res_p_label, sep = "*','")
srs_qri_scatterplot <- atypical_qri_df %>% 
    ggplot(aes(x = srs_ses, y = aQRI)) +
...
srs_qri_scatterplot 
ggsave(filename = here("outputs", "figs", "srs_qri_scatterplot_adjusted_ses.pdf"),
       plot = srs_qri_scatterplot, height = 4, width = 5.5)


# Distributions: QRI against 3 SRS Ranks ---------------------------------------
atypical_qri_df %>% 
    mutate(srs_rank = factor(ntile(srs_ses, 3))) %>% 
...
ggsave(filename = here("outputs", "figs", "srs_cortical_adjusted_ses_brainplot.pdf"),
...
ggsave(filename = here("outputs", "figs", glue("srs_subcortical_adjusted_ses_brainplot.pdf")),
...
ggsave(
    filename = here("outputs", "figs", "comparison_high_low_srs_adjusted_ses_brainplot.pdf"),
 ...
 atypical_qri_df %>% 
     select(contains(c("Left.Amygdala", "Putamen")), bblid, srs_ses) %>% 
     pivot_longer(cols = !c(bblid, srs_ses),
                  names_to = c("label"),
                  values_to = "QRI") %>% 
     group_by(bblid, srs_ses) %>% 
     summarize(
         network_qri = mean(QRI),
         .groups = "drop"
     ) %>% 
     select(srs_ses, network_qri) %>% 
     correlation::correlation(method = "spearman")
 ...
 ggsave(filename = here("outputs", "figs", "aqri_comparison_adjusted_ses_boxplot.pdf"),
